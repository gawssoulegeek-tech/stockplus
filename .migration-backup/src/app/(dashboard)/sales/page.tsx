      ...
      <Dialog open={detailDialogOpen} onOpenChange={setDetailDialogOpen}>
        <DialogContent className="sm:max-w-[600px] rounded-[2.5rem] p-0 overflow-hidden">
          <div className="p-8 space-y-6">
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-none bg-gray-50/50">
                      <TableHead className="font-bold text-gray-700 rounded-l-xl">Produit</TableHead>
                      <TableHead className="text-center font-bold text-gray-700">Qté</TableHead>
                      <TableHead className="text-right font-bold text-gray-700 rounded-r-xl">Total</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {detailItems.map((item) => (
                      <TableRow key={item.id} className="border-b border-gray-50">
                        <TableCell className="font-bold text-gray-800 py-4">{item.product_name}</TableCell>
                        <TableCell className="text-center font-bold text-gray-500">{item.quantity}</TableCell>
                        <TableCell className="text-right font-headline font-bold text-gray-900">{(item.item_total || item.unit_price * item.quantity).toLocaleString()} CFA</TableCell>
                      </TableRow>
                    ))}
            ...
            <div className="p-8 bg-orange-50/50 rounded-[2rem] flex justify-between items-center border border-orange-100">
              ...
              <span className="text-3xl font-headline font-bold text-primary">
                {detailSale?.total_amount?.toLocaleString()} CFA
              </span>
            </div>
          </div>
        </DialogContent>
